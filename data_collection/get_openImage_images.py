# Usage example: python3 get_openImage_images.py --classes 'Ice_cream,Cookie' --dataset train
# Actual cmd for all classes [train]: python3 get_openImage_images.py --classes 'Bathtub,Bed,Billiard table,Ceiling fan,Coffeemaker,Couch,Countertop,Dishwasher,Fireplace,Fountain,Gas stove,Jacuzzi,Kitchen & dining room table,Microwave oven,Mirror,Oven,Pillow,Porch,Refrigerator,Shower,Sink,Sofa bed,Stairs,Swimming pool,Television,Toilet,Towel,Tree house,Washing machine,Wine rack' --dataset train
# Actual cmd for all classes [validation]: python3 get_openImage_images.py --classes 'Bathtub,Bed,Billiard table,Ceiling fan,Coffeemaker,Couch,Countertop,Dishwasher,Fireplace,Fountain,Gas stove,Jacuzzi,Kitchen & dining room table,Microwave oven,Mirror,Oven,Pillow,Porch,Refrigerator,Shower,Sink,Sofa bed,Stairs,Swimming pool,Television,Toilet,Towel,Tree house,Washing machine,Wine rack' --dataset validation
...
with open("labels/class-descriptions-boxable.csv", mode="r") as infile:
...
for ind in range(0, len(classes)):
    ...
    command = "grep "+ dict_list[class_name.replace("_", " ")] + " labels/" + dataset + "-annotations-bbox.csv"
    print(command)
